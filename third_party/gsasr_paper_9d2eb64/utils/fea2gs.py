...
def window_partition(x, window_size):
    ...
    windows = x.reshape(
        b, c, h_count, window_size, w_count, window_size
    ).permute(0, 2, 4, 3, 5, 1).reshape(
        b * h_count * w_count, window_size * window_size, c
    )
    # h_count = h // window_size
    # w_count = w // window_size
...
def tokens_to_grid(tokens, h_count, w_count, token_side):
    batch = tokens.shape[0] // (h_count * w_count)
    channels = tokens.shape[-1]
    return tokens.reshape(
        batch, h_count, w_count, token_side, token_side, channels
    ).permute(0, 1, 3, 2, 4, 5).reshape(
        batch, h_count * token_side, w_count * token_side, channels
    )


def grid_to_tokens(grid, h_count, w_count, token_side):
...
class WindowCrossAttn(nn.Module):
    def __init__(self, dim=180, num_heads=6, window_size=12, num_gs_seed=2304):
        ...
        indexed_coords_delta = torch.tensor(
            indexed_coords_delta.sum(0).tolist(), dtype=torch.long
        )
...
class GSSelfAttnLayer(nn.Module):
    ...
    def forward(self, gs, pos, h_count, w_count, scale_embedding):
        ...
        #### shift gs
        if self.shift_size > 0:
            shift_gs = tokens_to_grid(
                gs, h_count, w_count, self.num_gs_seed_sqrt
            )
            shift_gs = torch.roll(shift_gs, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            shift_gs = grid_to_tokens(
                shift_gs, h_count, w_count, self.num_gs_seed_sqrt
            )
        else:
            shift_gs = gs
...
class Fea2GS(nn.Module):
    ...
    def forward(self, srcs, scale):
        ...
        query = tokens_to_grid(
            query,
            h // self.window_size,
            w // self.window_size,
            self.num_gs_seed_sqrt,
        ).permute(0, 3, 1, 2)
