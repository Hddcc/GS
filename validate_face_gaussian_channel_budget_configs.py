...
def main():
    ...
    if args.expected_channels not in (16, 32, 64):
        raise ValueError('Expected channels must be 16, 32, or 64.')
    ...
    if baseline.get('seed') != 1 or candidate.get('seed') != 1:
        errors.append('the budget screen is fixed to seed 1')
    ...
    if candidate_args.get('gaussian_channels') != args.expected_channels:
        errors.append('gaussian_channels does not match the requested budget')
    if candidate['epoch_max'] != 5 or candidate['epoch_save'] != 5:
        errors.append('candidate must use the five-epoch probe')
    if errors:
        raise ValueError('\n'.join(errors))

    print('GAUSSIAN CHANNEL-BUDGET CONFIG VALIDATION PASSED')
    ...
    print('bicubic bypass channels:', 64 - candidate_args['gaussian_channels'])
