GaussianSR IAFM IDM+ARM seed-1 bundle (2026-09-13)
...
-----------------------
face_iafm_idm_arm_seed1_bundle_20260913.zip

Run on the server
-----------------
cd /root/userfolder_new/20260527GaussiSR/GaussianSR-main
conda activate GS
unzip -q face_iafm_idm_arm_seed1_bundle_20260913.zip
PHYSICAL_GPUS=0,2,4 ADMISSION_GPU=4 EVALUATION_GPU=4 \
  bash face_iafm_idm_arm_seed1_bundle_20260913/install_and_launch_face_iafm_idm_arm_seed1_20260913.sh

The entrypoint verifies both SHA-256 manifests and the failed/completed
blind-SPD oracle terminal state. It accepts only the known server preimage or
an exact partial installation of this bundle, backs up the three overwritten
source files, runs CPU, single-GPU CUDA, and three-GPU DataParallel admission,
then launches training in the background.
