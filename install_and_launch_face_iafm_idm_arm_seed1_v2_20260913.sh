...
iafm_registration='from . import edsr_face_iafm_idm_arm'
server_init_preimage=25b730714c4afff8626ca3e5b3d946c6bb201ec13ab5a77a5a1e3018f77cd887

check_server_init() {
  local count actual filtered
  count=$(grep -cxF "$iafm_registration" models/__init__.py || true)
  [[ "$count" -le 1 ]] || {
    echo 'STOP: duplicate IAFM registrations in models/__init__.py' >&2
    exit 1
  }
  if [[ "$count" -eq 0 ]]; then
    actual=$(sha256sum models/__init__.py | cut -d ' ' -f1)
  else
    filtered=$(mktemp)
    grep -vxF "$iafm_registration" models/__init__.py > "$filtered"
    actual=$(sha256sum "$filtered" | cut -d ' ' -f1)
    rm -f "$filtered"
  fi
  [[ "$actual" == "$server_init_preimage" ]] || {
    echo 'STOP: models/__init__.py is not the diagnosed server version' >&2
    echo "expected-without-IAFM=$server_init_preimage" >&2
    echo "actual-without-IAFM=$actual" >&2
    exit 1
  }
}
...
targets=(
  ...
  README_face_iafm_idm_arm_seed1_v2_20260913.txt
...
for target in "${targets[@]}"; do
  ...
  case "$target" in
    models/gaussian.py|train_gaussian.py)
      ;;
    *)
      if [[ -e "$target" ]]; then
...
if ! grep -qxF "$iafm_registration" models/__init__.py; then
  printf '%s\n' "$iafm_registration" >> models/__init__.py
fi
check_server_init
[[ "$(grep -cxF "$iafm_registration" models/__init__.py)" -eq 1 ]] || {
  echo 'STOP: failed to install the IAFM model registration' >&2
  exit 1
}
