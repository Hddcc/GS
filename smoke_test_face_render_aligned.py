...
def main():
    ...
    print('channel duties:', encoder.render_channels, encoder.bypass_channels)
    print('input/query:', args.inp_size, args.sample_q)
    print(
        'initial gates: render={:.6f}, appearance={:.6f}, residual={:.6f}'
        .format(
            torch.sigmoid(encoder.render_gate_logit).item(),
    ...
    for scale_value in args.scales:
        ...
        assert_finite('render residual', encoder.last_render_residual)
        assert_finite('bypass residual', encoder.last_bypass_residual)
        assert_finite('render feature', encoder.last_render_feature)
        assert_finite('bypass feature', encoder.last_bypass_feature)
        ...
        if encoder.last_render_feature.shape[1] != 8:
            raise RuntimeError('Render path must contain exactly 8 channels.')
        if encoder.last_bypass_feature.shape[1] != 56:
            raise RuntimeError('Bypass path must contain exactly 56 channels.')
        render_magnitude = encoder.last_render_residual.abs().mean().item()
        bypass_magnitude = encoder.last_bypass_residual.abs().mean().item()
        if render_magnitude == 0 or bypass_magnitude == 0:
            raise RuntimeError('Render-aligned residuals must be nonzero.')
        ...
        render_grad = gradient_norm(
            encoder.render_fusion.parameters(),
            'render branch',
        )
        bypass_grad = gradient_norm(
            encoder.appearance_fusion.parameters(),
            'bypass branch',
        )
        render_gate_grad = gradient_norm(
            [encoder.render_gate_logit],
            'render gate',
        )
        ...
        print(
            ...
            'render={:.8f}, bypass={:.8f}, rgb_residual={:.8f}, '
            'grads=geometry:{:.3e}/render:{:.3e}/bypass:{:.3e}/'
            'render_gate:{:.3e}/appearance_gate:{:.3e}/residual:{:.3e}'
            .format(
                scale_value,
                tuple(pred.shape),
                loss.item(),
